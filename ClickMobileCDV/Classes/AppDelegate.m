...
{
    [self getAppVersion];
    [self getPhoneNumber];//Added to fetch phone number
    ...
    bool bclearAppliactionCache_preference = [userDefaults boolForKey:@"clearCache_preference"];
...
//Added for phone number sync issue
-(void) getPhoneNumberTimer:(NSTimer*) t{
    NSLog(@"Inside AppDelegate: getPhoneNumberTimer t");
    [self getPhoneNumber];
}
//For Getting App version added by Souvik
- (void *)getAppVersion
{
    NSLog(@"Inside AppDelegate: getAppVersion");
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    
    //Getting version number.
    NSString* version = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    NSLog(@"{AppDelegate} App version is: %@",version);
    //Getting Bundle number
    //NSString* bundle = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleVersion"];
    //Setting version number.
    [defaults setObject:version forKey:@"settingsVersion"];
    //----------End--------------
    
}
- (NSString *)getPhoneNumber
{
    NSLog(@"Inside AppDelegate: getPhoneNumber");
    //NSLog(@"getPhoneNumber WITHOUT TIMER %d",loadCount);
    // The first thing we want to do is check the application settings for the existence
    // of the phone number
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    __block NSString *phoneNum = [defaults stringForKey:@"settingsPhoneNumber"];
...
//                     phoneNum = @"111111111111";
                     phoneNum = @"";
                     NSLog(@"{MainViewController} Webservice not returning any JSON");
                     [defaults setValue:phoneNum forKey:@"settingsPhoneNumber"];
...
//        [defaults setValue:phoneNum forKey:@"settingsPhoneNumber"];
        ...
        NSLog(@"{MainViewController} Device Phone Number from app settings: %@", phoneNum);
    }
    //[defaults setValue:phoneNum forKey:@"settingsPhoneNumber"];
    NSMutableString *stringToReturn = [NSMutableString stringWithString:@""];
    if (phoneNum != nil) {
        // Append the phone number to stringToReturn
        [stringToReturn appendString:phoneNum];
    }
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary*)launchOptions
{
    ...
    // If the number doesn't exist in settings, try to retrieve it from the device
    if ([phoneNum length] == 0) {
        [self.viewController.webViewEngine loadRequest:url];
    }else{
        modifyReqURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@?LWSdeliverymobilephnum=%@", url, [[NSUserDefaults standardUserDefaults] stringForKey:@"settingsPhoneNumber"]]];
    }
...
//    NSString *phoneNum = [defaults stringForKey:@"settingsPhoneNumber"];
    NSTimer* myTimer;
    //Setting up the timer based on phone number availability.
    if ([phoneNum length] == 0) {
        //Added for phone number sync issue
        myTimer = [NSTimer scheduledTimerWithTimeInterval: 3600.0 target: self
                                                 selector: @selector(getPhoneNumberTimer:) userInfo: nil repeats: YES];
    }else{
        [myTimer invalidate];
    }
